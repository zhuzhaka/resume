...
export const CLoseDropLists = () => {
  document.querySelectorAll('.dropdown-list__items-wrapper_dropped')
    .forEach(item => {
      ...
      item.closest('.dropdown-list_dropped')?.classList.remove('dropdown-list_dropped');
    });
}
    ...
    document.querySelectorAll('.' + listDroppedClassName)
      .forEach(element => {
        if (currentElement !== element) {
          ...
          element.closest('.dropdown-list_dropped')?.classList.remove('dropdown-list_dropped');
        }
      }
      );
  ...
  return (
    ...
    >
      <div className="dropdown-list__header">
        <div className="dropdown-list__selected">
          {selectedItem.icon && showIcon ? (typeof selectedItem.icon === 'function' ? selectedItem.icon() : selectedItem.icon) : ''}
          {showTitle && <span className="dropdown-list__selected-title">{selectedItem.title}</span>}          
        </div>
      ...
      >
        {items.map(({ title, icon, dataAttributes }) => (
          ...
          >
            {icon && showIcon ? (typeof icon === "function" ? icon() : icon) : ""}
